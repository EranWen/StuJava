	 ...
	 private String							MidPathStr;
	  //隐藏文件过滤器
		...
		{	
			...
			conf = job;
			System.out.println("获取Splits");
			MidPathStr=job.get("mapreduce.splitvideofile.midPath","");
			ArrayList<FileSplit> splits = new ArrayList<FileSplit>();
			SplitFileList = new ArrayList<String>();
			
			Path log = new Path(MidPathStr+"log.log");
			FileSystem logfs = log.getFileSystem(job);
			FSDataOutputStream logStream = logfs.create(log, true,1024);
			logStream.writeBytes(MidPathStr+"\n");
			...
			logStream.writeBytes("Split"+size);
			System.out.println("获取Splits完成");
			jobfile.close();
			logStream.close();
